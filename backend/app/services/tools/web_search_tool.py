"""Web search tool — separated from LLM service."""
...
def wants_web_search(message: str) -> bool:
    """True when message explicitly asks for research or current facts."""
    text = message.strip()
    if len(text) < 10:
        return False
...
async def fetch_web_context(query: str, max_chars: int = 800) -> str:
    """Fetch lightweight web hints via DuckDuckGo instant answer API."""
    if not wants_web_search(query):
        return ""
    q = query.strip()[:200]
    if not q:
        return ""
